...
function outdated (args, silent, cb) {
  ...
  readPackageTree(dir, andComputeMetadata(function (er, tree) {
    ...
    outdated_(args, '', tree, {}, 0, opts, function (er, list) {
      ...
      const levels = ['none', 'wanted', 'latest']
      const minLevel = levels.indexOf(opts['outdated-level'])
      const outdatedCount = list.reduce((count, dependency) => {
        return semver.lt(dependency[2], dependency[2 + minLevel]) ? count + 1 : count
      }, 0)
      process.exitCode = outdatedCount ? 1 : 0
